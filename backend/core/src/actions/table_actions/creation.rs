use common::sea_orm::{ActiveEnum, ConnectionTrait, DbErr, EntityTrait, ExecResult, Schema};
...
pub async fn create_type<E: ActiveEnum>(db_connection: &common::sea_orm::DatabaseConnection) -> Result<ExecResult, DbErr>{
    let backend = db_connection.get_database_backend();
    let schema = Schema::new(backend);
    db_connection
